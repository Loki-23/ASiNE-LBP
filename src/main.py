...
class ASiNE(object):
    ...
    def __init__(self, config):
        ...
        self.pos_graph, self.neg_graph, self.n_node, self.pos_roots, self.neg_roots = \
            utils.read_edges(
                self.config.train_filename, self.config.test_filename)

        self.train_graph = utils.read_edges_from_file(
            self.config.train_filename)
        self.test_graph = utils.read_edges_from_file(self.config.test_filename)

        init_delta_D = 0.05
        init_delta_G = 0.05
        self.node_emb_init_d = tf.Variable(tf.random.uniform(
            [self.n_node, self.config.n_emb], minval=-init_delta_D, maxval=init_delta_D, dtype=tf.float32))
        self.node_emb_init_g = tf.Variable(tf.random.uniform(
            [self.n_node, self.config.n_emb], minval=-init_delta_G, maxval=init_delta_G, dtype=tf.float32))
    ...
    def train(self):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            pos_roots_parted = list(
                utils.divide_chunks(self.pos_roots, root_parts))

            if epoch > 0 and epoch % self.config.save_steps == 0:   # save the model
                self.saver.save(
                    self.sess, self.config.model_log + ".model.checkpoint")

            print("\t Performing adversarial learning in positive graph...")
            for roots in pos_roots_parted:
                self.pos_partition_trees = self.construct_trees(
                    self.pos_graph, roots)
                ...
                for d_epoch in range(self.config.n_epochs_dis):
                    center_nodes, neighbor_nodes, labels = pos_dis_vars[
                        0], pos_dis_vars[1], pos_dis_vars[2]

                    train_size = len(center_nodes)
                    start_list = list(
                        range(0, train_size, self.config.batch_size_dis))
                ...
                for g_epoch in range(self.config.n_epochs_gen):
                    ...
                    start_list = list(
                        range(0, train_size, self.config.batch_size_gen))
            ...
            neg_roots_parted = list(
                utils.divide_chunks(self.neg_roots, root_parts))

            print("\t Performing adversarial learning in negative graph...")
            for roots in neg_roots_parted:
                self.neg_partition_trees = self.construct_trees(
                    self.neg_graph, roots)
                neg_dis_vars, neg_gen_vars, pos_dis_vars, pos_gen_vars = self.prepare_data_negative(
                    roots)
                ...
                dis_all_cnt = 0
                for d_epoch in range(self.config.n_epochs_dis):
                    center_nodes, neighbor_nodes, labels = neg_dis_vars[
                        0], neg_dis_vars[1], neg_dis_vars[2]
                ...
                if self.config.learn_fake_pos == True:
                    for g_epoch in range(self.config.n_epochs_gen):
                        pos_node_1, pos_node_2, pos_reward = pos_gen_vars[
                            0], pos_gen_vars[1], pos_gen_vars[2]
            ...
            pos_precision, neg_precision = self.evaluation(self, epoch)
            final_pos = final_pos+pos_precision
            final_neg = final_neg+neg_precision
        self.write_embeddings_to_file()
        final_pos = final_pos/20
        final_neg = final_neg/20
    ...
    def prepare_data_positive(self, roots):
        ...
        for i in roots:
            if np.random.rand() < self.config.update_ratio:
                ...
                fake, paths_from_i = self.pos_sample(
                    i, self.pos_partition_trees[i], n_sample, for_d=True)
    ...
    def prepare_data_negative(self, roots):
        ...
        for i in roots:
            if np.random.rand() < self.config.update_ratio:
                ...
                n_sample = max(len(real), self.config.n_sample_gen)
                neg_fakes, pos_fakes, paths_from_i = self.neg_sample(
                    i, self.neg_partition_trees[i], n_sample, for_d=True)
                if paths_from_i is None:
                    continue
        ...
        gen_node_pairs_sign = list(
            map(self.get_node_pairs_sign_from_path, gen_paths))
        ...
        for i_path in range(len(gen_node_pairs)):  # i-th pair
            for j_pair in range(len(gen_node_pairs[i_path])):
                if gen_node_pairs_sign[i_path][j_pair] == [-1]:
                    neg_gen_node_pair_1.append(
                        gen_node_pairs[i_path][j_pair][0])
                    neg_gen_node_pair_2.append(
                        gen_node_pairs[i_path][j_pair][1])
                else:
                    if self.config.learn_fake_pos == True:
                        pos_gen_node_pair_1.append(
                            gen_node_pairs[i_path][j_pair][0])
                        pos_gen_node_pair_2.append(
                            gen_node_pairs[i_path][j_pair][1])
    ...
    def neg_sample(self, root, tree, sample_num, for_d):
        ...
        while len(neg_fakes) < sample_num:
            ...
            while True:
                ...
                relevance_probability = target_score[0, node_neighbor]
                relevance_probability = np.nan_to_num(relevance_probability)
                relevance_probability = utils.softmax(
                    1 - (utils.softmax(relevance_probability)))
                ...
                if next_node == previous_node:  # terminating condition
                    ...
                    if self.config.learn_fake_pos == True:
                        if len(paths[n]) % 2 == 1:  # if len(path) is odd, sample next node
                            ...
                            # if len(path) is odd, sample current node
    ...
    def write_embeddings_to_file(self):
        ...
        modes = [self.pos_generator, self.pos_discriminator,
                 self.neg_generator, self.neg_discriminator]
    ...
    def evaluation(self, epoch):
        ...
        for i in range(len(modes)):
            ...
            cntn = 0
            tot = 0
            for x in range(len(test_y_pred)):
                tot = tot+1
                if Y_test[x] == test_y_pred[x]:
                    if Y_test[x] == 1:
                        cntp = cntp+1
                    else:
                        cntn=cntn+1
            print(cntp," ",cntn)
            pos_Precision = cntp/tot
            neg_Precision = cntn/tot
...
def parse_args():
    ...
    res_fn_path = "./results/" + args.dataset + \
        "_dim" + str(args.n_emb) + "_lr" + str(args.lr)
